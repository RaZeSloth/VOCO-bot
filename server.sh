...
sed -i "s/puppeteer/puppeteer-core/g" src/util/functions.ts && sed -i "s_({ headless: true, defaultViewport: { width: 1920, height: 700 } });_({ headless: true, executablePath: '/usr/bin/chromium-browser', defaultViewport: { width: 1920, height: 700 } });_g" src/util/functions.ts
