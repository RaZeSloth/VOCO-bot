...
export = async (client: VocoBot) => {
	...
	client.on('ready', async () => {
		...
		// (client.channels.cache.get('1029435516644507768') as TextBasedChannel).send({ content: 'Vajuta nuppu', components: [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setLabel('Tunni ping').setStyle(ButtonStyle.Primary).setCustomId('add_role_tund'))] });
