	...
	if (int.isButton() && int.inCachedGuild()) {
		...
		case 'add_role_tund':
			if (int.member?.roles.cache.has('1029335363040329749')) {
				...
				int.reply({ content: 'Eemaldasin <@&1029335363040329749> role!', flags: MessageFlags.Ephemeral });
			} else {
				int.member.roles.add('1029335363040329749');
				int.reply({ content: 'Lisasin <@&1029335363040329749> role!', flags: MessageFlags.Ephemeral });
			}
			break;
		case 'add_role_buss':
			if (int.member?.roles.cache.has('1066785642115235900')) {
				int.member.roles.remove('1066785642115235900');
				int.reply({ content: 'Eemaldasin <@&1066785642115235900> role!', flags: MessageFlags.Ephemeral });
			} else {
				int.member.roles.add('1066785642115235900');
				int.reply({ content: 'Lisasin <@&1066785642115235900> role!', flags: MessageFlags.Ephemeral });
			}
			break;
		}
	}
